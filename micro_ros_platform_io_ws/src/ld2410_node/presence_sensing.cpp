...
std::unique_ptr<radar_lld::RadarLd2410Manager<NUM_SENSORS>> radar_node;

void setup(void)
{

  /*Order of config decides the sensor id. So sensor with uart 1 in this case will get id 0*/
  const std::array<radar_lld::UartConfig, NUM_SENSORS> radarConfigs = {{
    ...
    const radar_lld::UartConfig ros_serial_config {0,0,0,SERIAL_BAUDRATE}; //UART=0, Baudrate=256000 
    radar_node.reset(new radar_lld::RadarLd2410Manager<NUM_SENSORS>(NODE_NAME, RADAR_PUBLISH_TOPIC_NAME, radarConfigs, ros_serial_config, DEVICE_ID, PIN_LED_INDICATOR));
  #elif defined(MICRO_ROS_TRANSPORT_ARDUINO_WIFI) //Start node in wifi coumminciation mode.
    const radar_lld::WifiConfig wifi_config {AGENT_PORT, AGENT_IP, WIFI_SSID, WIFI_PASSWORD};
    radar_node.reset(new radar_lld::RadarLd2410Manager<NUM_SENSORS>(NODE_NAME, RADAR_PUBLISH_TOPIC_NAME, radarConfigs, wifi_config, DEVICE_ID, PIN_LED_INDICATOR));
  #endif
}
